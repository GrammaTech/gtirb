...
TEST(Unit_Utilities, ByteArray8To16) {
  ...
  ASSERT_NO_THROW(ByteArray8To16(input));

  const auto output = ByteArray8To16(input);
...
TEST(Unit_Utilities, ByteArray8To16Swap) {
  ...
  const auto output = ByteArray8To16(input, true);
...
TEST(Unit_Utilities, ByteArray8To16Error) {
  ...
  EXPECT_THROW(ByteArray8To16(input), std::range_error);
}
