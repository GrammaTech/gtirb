...
public:
  ...
  Error(std::unique_ptr<ErrorInfoBase> Pld) {
    setPtr(Pld.release());
    setChecked(false);
  }
...
private:
  ...
  std::list<std::unique_ptr<ErrorInfoBase>> Payloads;
};
...
inline void cantFail(Error Err, const char* Msg = nullptr) {
  if (Err) {
    ...
    Msg = OS.str().c_str();
#endif
    assert(false && "cantFail should be unreachable");
  }
}
...
void logAllUnhandledErrors(Error E, std::ostream& OS,
...
public:
  ErrorAsOutParameter(Error* Error) : Err(Error) {
...
public:
  ExpectedAsOutParameter(Expected<T>* MaybeValue) : ValOrErr(MaybeValue) {
    if (ValOrErr)
      (void)!!*ValOrErr;
  }
...
public:
  void setErrorCode(std::error_code ErrCode) { this->EC = ErrCode; }
...
public:
  ...
  StringError(std::error_code EC, const char* S = "");
...
class FileError final : public ErrorInfo<FileError> {
  ...
  friend Error createFileError(const char*, Error);
  friend Error createFileError(const char*, size_t, Error);
...
private:
  FileError(const char* F, std::optional<size_t> LineNum,
  ...
  static Error build(const char* F, std::optional<size_t> Line, Error E) {
...
/// Error is unchecked.
inline Error createFileError(const char* F, Error E) {
...
inline Error createFileError(const char* F, size_t Line, Error E) {
...
inline Error createFileError(const char* F, std::error_code ErrCode) {
  return createFileError(F, errorCodeToError(ErrCode));
}

/// Concatenate a source file path and/or name with line number and
/// std::error_code to form an Error object.
inline Error createFileError(const char* F, size_t Line,
                             std::error_code ErrCode) {
  return createFileError(F, Line, errorCodeToError(ErrCode));
}

Error createFileError(const char* F, ErrorSuccess) = delete;
...
public:
  ...
  ExitOnError(std::string ExitBanner = "", int DefaultErrorExitCode = 1)
      : Banner(std::move(ExitBanner)),
        GetExitCode([=](const Error&) { return DefaultErrorExitCode; }) {}

  /// Set the banner string for any errors caught by operator().
  void setBanner(std::string NewBanner) { this->Banner = std::move(NewBanner); }

  /// Set the exit-code mapper function.
  void setExitCodeMapper(std::function<int(const Error&)> ExitCodeMapper) {
    this->GetExitCode = std::move(ExitCodeMapper);
  }
