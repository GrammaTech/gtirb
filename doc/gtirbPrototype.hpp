...
// - Non-class enums can use `UserDefined` as their last value to indicate a starting offset for
// user-defined values.
...
// - Doxygen-style documentation should be provided for each class and each public function within
// the class.
// - `#ifdef` to conditionally compile code is not desireable.  It should be avoided especially in
// header files.
...
// - Do not hold raw pointers external to the IR.  Use them with a local scope (limited lifetime)
// only.
//
...
namespace gtirb
{
    ...
    /// \class LogicError
    ///
    /// Defines a type of object to be thrown as exception. It reports errors that are a consequence
    /// of
    /// faulty logic within the program such as violating logical preconditions or class invariants
    /// and
    /// may be preventable.
    ///
    ...
    /// While some data may be very node-specific, the table stores arbitrary data that spans many
    /// nodes.
    ///
    template <typename R = std::weak_ptr<gtirb::Node>, typename C = std::string,
              typename T = std::any>
    class Table
    {
    public:
    ...
    public:
        ...
        ///             2)  Returns exactly std::begin(c), with c always treated as const-qualified.
        ///             If
        ...
        template <typename T>
        TypedNodeIterator<T> begin();
        template <typename T>
        ConstTypedNodeIterator<T> begin() const;

        ///
        /// Returns a custom iterator to the end of the child nodes.
        /// API is modeled after the STL.
        ///
        /// \return     Returns an iterator to the end (i.e. the element after the last element) of
        /// the
        ...
        template <typename T>
        TypedNodeIterator<T> end();
        template <typename T>
        ConstTypedNodeIterator<T> end() const;
        ...
        /// Starting at this node, will search "up" the tree until the given table is found or
        /// return null.
        ///
    ...
    public:
        ...
        constexpr FaceFeed = 0xcefaedfe
            ...
            /// 0xDEADBEEF
            /// ("dead beef") is used by IBM RS/6000 systems, Mac OS on 32-bit PowerPC processors
            /// and the
            /// Commodore Amiga as a magic debug value. On Sun Microsystems' Solaris, it marks freed
            /// kernel
            /// memory.
    ...
    public:
        ...
        public:
            ...
            public:
                ...
                /// \class Symbol
                ///
                /// Users can inherit from this and build their own custom symbol types, if
                /// required.
            ...
            /// disassembly.  Anything that is needed after disassembly should properly be in the
            /// CFG,
            /// at which point, this is no longer needed. However, currently, fProcInfo is very
            /// pervasive.  How much of that is just for information that we would move to CFG, I
            /// don't
            /// know.
            ///
            ...
            public:
                ...
                /// \param x A reference owned externally as a shared_ptr.
                ///
                std::map<gsl::not_null<gtirb::Symbol*>, gsl::not_null<gtirb::Symbol*>>&
                getSymbolToSymbolMap();
                const std::map<gsl::not_null<gtirb::Symbol*>, gsl::not_null<gtirb::Symbol*>>&
                getSymbolToSymbolMap() const;
                ...
                ///
                std::map<gsl::not_null<gtirb::Symbol*>, gsl::not_null<gtirb::Symbol*>>&
                getSaveToRestoreSymbolMap();
                const std::map<gsl::not_null<gtirb::Symbol*>, gsl::not_null<gtirb::Symbol*>>&
                getSaveToRestoreSymbolMap() const;
                ...
                const std::map<gtirb::EA, gsl::not_null<gtirb::RegionHeap*>>& getEAToHeapRegionMap()
                    const;
            ...
            public:
                ...
                public:
                    enum Kind
                    {
                        ...
                        Switch,
                        VarDecl,
                        UserDefined /// The last slot in the enumeration so users can add their own
                                    /// types
            ...
            public:
                ...
                /// These entries are basically the "thunks" for calls to things in shared
                /// libraries.
                ///
            ...
            ///
            /// Extract an eight bit vector of data starting at a given EA for a given number of
            /// bytes.
            ///
            std::vector<uint8_t> getChildrenBytes8(gtirb::EA x, size_t nbytes) const;
            std::vector<uint8_t> getChildrenBytesUntil(
                gtirb::EA x, size_t nbytes, std::function<bool(uint8_t)> passFunction) const;
        ...
        /// Each validation function gets a pointer to its node and a stream to output error
        /// messages on.
        ///
        /// \return true if all validation functions returned true.
        ///
        bool applyValidation(
            std::vector<std::function<bool(const Node* const, ostream& os = std::cerr)>>
                validators);
    };
...
namespace gtirb
{
    namespace utilities
    {
        ...
        void PrettyPrint(gsl::not_null<const gtirb::IR* const> x,
        ...
        ///
        /// \return         Check the return tuple values for their "Unknown" state on failure. The
        /// return
        ///                 tuple's integer value will be set only for ELF format files (nbits).
        ///
        std::tuple<gtirb::FileFormat, gtirb::FileFunction, gtirb::ISAID, int> GetFileFormat(
            const std::filesystem::path& p, int64_t offset = 0);
...
namespace gtProprietary
{
    ...
    AnalysisEnvironment* GTIRB2AnalysisEnvironment(gsl::not_null<const gtirb::IR* const> x,
                                                   FATOBJ_STORE_ID store_id);
    ...
    std::unique_ptr<gtirb::IR> AnalysisEnvironment2GTIRB(
        gsl::not_null<const AnalysisEnvironment* const> x);
}
...
void DecorateAllSymbols2()
{
    ...
    {
        std::for_each(symbolsNode->begin<Symbol>(), symbolsNode->end<Symbols>()[](Symbol * s) {
            auto foo = std::make_unique<Foo>();
            foo->setMyData(rand());
            s->push_back(foo);
        });
